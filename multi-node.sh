...
{
ssh -o StrictHostKeyChecking=no -vvv -T -i ~/${slave_keypair} ${ami[1]}@${INSTANCE_IPS[0]} <<-EOF && { echo "Build success on ${INSTANCE_IPS[$1]}" ; return 0 ; } || { echo "Build failed on ${INSTANCE_IPS[$1]}"; return 1 ;  }
